...
panel.background = element_blank(), axis.line = element_line(colour = "black"),
    ...
    annotate("text", x=0.01*max(tmaxrk),y=0.975*1.25, label="E")
}
...
## Output:
## - a ggplot2 plot
plot_density <- function(dat, letter) {
    ggplot(dat) +
...
panel.background = element_blank(), axis.line = element_line(colour = "black"),legend.position="none") + 
    annotate("text",x=0.01*max(tmaxrk),y =0.975*3.3, label=letter)+
	geom_text(x=(-1+2*.05), y=30, label="Scatter plot")%>%
        return
}
...
## Output:
## - a ggplot2 plot
plot_trait <- function(dat, letter) {
    ggplot(dat) +
        ...
        ylab("trait value") +
        annotate("text",x=0.01*max(tmaxrk),y =0.97*.5, label=letter)+
        theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black"), legend.position="none") %>%
        return
}
...
## Output:
## - a ggplot2 plot
plot_snapshot <- function(dat, moment=0, limits=c(-1, 1, NA), title=as.character("hey"), res=501, letter="") {
    ...
    landscape <- data_frame(trait=traitaxis, r=0) ## for plotting intrinsic rates
    ...
    if (!is.na(limits[3])) landscape$r <- landscape$r * limits[3] * 0.98
        ...
        theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black"),legend.position="none") +
    annotate("text",x=(min(traitaxis)+.01*(max(traitaxis)-min(traitaxis))),y =1.01*(max(traits$density)), label=letter)+
        ggtitle(label=title)  %>% 
        return
}
...
plot_time<-function(dat, parms) {
    plot_grid(plot_density(dat, "C"), plot_trait(dat, "D"), plot_rhokappa(dat,parms), ncol=1, align="v") %>%
        return
}

plot_all <- function(dat, parms, moment=0, limits=c(-1, 1, NA), title, res=501) {
    plot_grid(plot_snapshot(dat, moment=0, limits, title=c(paste("time=0")), res, letter="A"),
              plot_snapshot(dat, moment=extrk$time, limits, title= c("time=100"), res, letter="B"),  plot_time(dat,parms), ncol=3) %>%
        return
}
